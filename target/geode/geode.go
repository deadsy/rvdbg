...
Geode is a development platform using a BCM47722
...
import (
	...
	"github.com/deadsy/rvdbg/chip/broadcom/bcm47722"
...
var Info = target.Info{
	Name:     "geode",
	Descr:    "Geode Board (Broadcom BCM47722, Quad Core ARM 64-bit)",
...
func New(jtagDriver jtag.Driver) (target.Target, error) {
	...
	// make the jtag chain
	jtagChain, err := jtag.NewChain(jtagDriver, bcm47722.Chain)
	if err != nil {
		return nil, err
	}

	// make the jtag device for the cpu core
	jtagDevice, err := jtagChain.GetDevice(bcm47722.CoreIndex)
	if err != nil {
		return nil, err
	}
